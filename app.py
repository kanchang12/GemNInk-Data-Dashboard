...
from multi_csv import process_multiple_csvs
...
def upload_multiple_files():
    """Upload and process multiple CSV files with auto-join"""
    try:
        # Get multiple files from request
        uploaded_files = request.files.getlist('files[]')  # Note: files[] for multiple
        
        if not uploaded_files or all(f.filename == '' for f in uploaded_files):
            return jsonify({'error': 'No files selected'}), 400
        
        # Process multiple CSVs
        merged_df, join_info, error = process_multiple_csvs(uploaded_files)
        
        if error:
            return jsonify({'error': f'Failed to process files: {error}'}), 400
        
        if merged_df is None:
            return jsonify({'error': 'No valid data found in uploaded files'}), 400
        
        # Same automated processing as your single file upload
        initial_rows = merged_df.shape[0]
        initial_missing = merged_df.isnull().sum().sum()

        # Apply your existing data cleaning
        cleaned_df, cleaning_report_message = data_cleaning.clean_data(merged_df.copy())
        
        # Apply outlier removal if there are numerical columns
        numerical_cols = cleaned_df.select_dtypes(include=np.number).columns
        total_outliers_removed = 0
        
        if len(numerical_cols) > 0:
            original_rows_before_outlier_removal = cleaned_df.shape[0]
            cleaned_df, outlier_message = data_cleaning.remove_outliers(
                cleaned_df, columns=list(numerical_cols), method='iqr', z_thresh=5
            )
        ...
        # Generate insights using your existing functions
        ...
        all_insights = {
            ...
            "join_info": join_info  # Add the join information
        }
        
        # Generate AI summary for multi-table dataset
        ai_insights_summary = "AI summary generation skipped."
        if vertex_ai_model:
            try:
                files_info = ', '.join([f.filename for f in uploaded_files if f.filename])
                ai_analysis_prompt = f"""
                You are an expert data analyst. This dataset was created by joining multiple CSV files: {files_info}.
                
                Join Information: {join_info}
                
                Based on the analysis results, provide a concise overview (100-150 words) highlighting:
                - How the tables were joined
                - Key insights from the merged dataset
                - Data quality after merging
                
                Analysis Results: {json.dumps(all_insights, indent=2)}
                """
                chat_session_auto = vertex_ai_model.start_chat()
        ...
        # Generate dashboard
        dashboard_explanation = f"Multi-table analysis of {len(join_info.get('tables_info', {}))} merged datasets"
        ...
        dashboard_filename = f"multi_dashboard_{uuid.uuid4().hex}.html"
        ...
        # Store in session exactly like single file
        dataset_id = str(uuid.uuid4())
        session[dataset_id] = {
            'df': cleaned_df.to_dict('records'),
            'info': get_df_info(cleaned_df, f"Merged Dataset ({len(uploaded_files)} files)"),
            'chat_history': [],
            'upload_timestamp': datetime.now().isoformat(),
            'dashboard_url': dashboard_url,
            'initial_insights': all_insights,
            'is_multi_table': True,  # Flag to indicate this is multi-table
            'join_info': join_info
        }
        session['active_dataset_id'] = dataset_id
        
        cleaning_message = (f"Multi-table processing complete! "
                          f"Merged {len(uploaded_files)} files. "
                          f"Final rows: {rows_after_cleaning:,}, Missing values: {missing_after_cleaning:,}")
        
        return jsonify({
            'message': 'Multiple files processed and merged successfully!',
            ...
            'initial_insights': all_insights,
            'join_info': join_info,
            'files_processed': [f.filename for f in uploaded_files if f.filename]
        }), 200
        
    except Exception as e:
        app.logger.error(f"Error processing multiple files: {traceback.format_exc()}")
        return jsonify({'error': f'Failed to process multiple files: {str(e)}'}), 500
